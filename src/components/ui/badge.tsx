...
const badgeVariants = cva(
  ...
  ],
  {
    variants: {
      variant: {
        default: 'border font-medium bg-foreground/5',
        primary:
...
function Badge({
  ...
  return (
    <Comp
      ...
      {...props}
    >
      { icon && <Icon icon={icon} className="inline-block size-[18px]" /> }
      <span className="pt-px text-[13px] font-[550] pl-2 pr-1.5">
        { props.children }
      </span>
    </Comp>
  )
